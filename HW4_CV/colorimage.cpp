...
MonoImage* ColorImage::GetGrey2(ColorImage *cimg)
{
    MonoImage* grey;
    int cols = cimg->cols;
    int rows = cimg->rows;
...
ColorImage* ColorImage::Subtract2MonoImage(MonoImage *img1, MonoImage *img2)
{
    ColorImage* contour;
    int*** pixels;
    int rows = img1->rows;
    ...
    // Pontential leak of mamory, should be fixed.
    contour = new ColorImage(pixels, rows, cols);
    return contour;
}
